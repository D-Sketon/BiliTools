...
class Dialog_Week(QDialog):
    reload = QtCore.pyqtSignal(str)

    def __init__(self):
        super(Dialog_Week, self).__init__()
        self.setupUi()
    ...
    def retranslateUi(self, Dialog_Week):
        ...
        Dialog_Week.setWindowTitle(_translate("Dialog_Week", "每周必看"))
        self.label.setText(_translate("Dialog_Week", "周数   "))
    ...
    def accept(self):
        week = self.lineEdit.text()
        if len(week) == 0:
            self.reject()
        self.reject()
        self.reload.emit(week)
